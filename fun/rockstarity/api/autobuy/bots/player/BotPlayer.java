...
public class BotPlayer extends AbstractClientPlayerEntity {
	...
	@Getter final TimerUtility updateTimer = new TimerUtility();
	@Getter final TimerUtility antiAfkTimer = new TimerUtility();
    ...
    public boolean FORWARD, BACKWARD, LEFT, RIGHT, JUMP, SNEAK;
    
    public Screen currentScreen;

    /**
     * The last X position which was transmitted tfo the server, used to determine when the X position changes and needs
     * to be re-trasmitted
     */
       ...
       // if (this.isCurrentViewEntity())
        {
            ...
            if (Math.abs(this.getPosX()) < 1) return;
            ...
            this.autoJumpEnabled = this.mc.getGameSettings().autoJump;
        }
    }
    ...
    public void addChatMessage(String msg) {
    	 //  this.mc.ingameGUI.getChatGUI().printChatMessage(new TranslationTextComponent(msg + ""));
    }
    ...
    public void livingTick()
    {
        ...
        {
            if (this.sprintToggleTimer <= 0 && !this.mc.getGameSettings().keyBindSprint.isKeyDown())
            {
        ...
        if (!this.isSprinting() && (!this.isInWater() || this.canSwim()) && this.isUsingSwimmingAnimation() && flag4 && !this.isHandActive() && !this.isPotionActive(Effects.BLINDNESS) && this.mc.getGameSettings().keyBindSprint.isKeyDown())
        {
            this.setSprinting(true);
        }
    ...
    {
        return this.isAutoJumpEnabled() && this.autoJumpTime <= 0 && this.onGround && !this.isSneaking() && !this.isPassenger() && Move.isMoving() && (double)this.getJumpFactor() >= 1.0D;
    }
    ...
    {
        if (this.mc.getGameSettings().getPointOfView().func_243192_a())
        {
