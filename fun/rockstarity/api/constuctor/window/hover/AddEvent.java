...
public class AddEvent extends Window implements IAccess {
	...
	public AddEvent(float x, float y, float width, float height) {
		...
		for (Entry<String, Class> action : rock.getScriptConstructor().getEvents().entrySet()) {
			actions.put(action.getKey(), () -> {
				rock.getScriptConstructor().getBlocks().add(new BlockEvent(action.getKey(), x,y,100,35, action.getValue()));
			});
		}
	}
	...
	public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
		...
		bold.get(14).draw(matrixStack, "Событие при", x + 5, y + 4, black);
		float yOff = 0;
