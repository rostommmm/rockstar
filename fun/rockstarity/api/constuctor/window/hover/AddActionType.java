...
public class AddActionType extends Window implements IAccess {
	...
	public AddActionType(float x, float y, float width, float height) {
		...
		for (Entry<String, HashMap> action : rock.getScriptConstructor().getActionTypes().entrySet()) {
			actions.put(action.getKey(), () -> {
				rock.getScriptConstructor().getScreen().getWindows().add(new AddAction(action.getKey(), x+width+5, y, 125, 100));
			});
		}
	}
	...
	public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
		...
		bold.get(14).draw(matrixStack, "Событие", x + 5, y + 4, black);
		float yOff = 0;
