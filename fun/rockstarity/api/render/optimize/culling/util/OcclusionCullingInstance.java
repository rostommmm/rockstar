...
import fun.rockstarity.api.render.optimize.culling.util.cache.ArrayOcclusionCache;
import fun.rockstarity.api.render.optimize.culling.util.cache.OcclusionCache;
import fun.rockstarity.api.render.optimize.culling.util.util.MathUtilities;
...
public class OcclusionCullingInstance {
    ...
    private final Vector3d[] targetPoints = new Vector3d[15];
    private final Vector3d targetPos = new Vector3d(0, 0, 0);
    ...
    public OcclusionCullingInstance(int maxDistance, DataProvider provider, OcclusionCache cache, double aabbExpansion) {
        ...
        for(int i = 0; i < targetPoints.length; i++) {
            targetPoints[i] = new Vector3d(0, 0, 0);
        }
    }

    public boolean isAABBVisible(Vector3d aabbMin, Vector3d aabbMax, Vector3d viewerPosition) {
        try {
     ...
     * @return
     */
    private boolean isVoxelVisible(Vector3d viewerPosition, Vector3d position, byte faceData, byte visibleOnFace) {
        int targetSize = 0;
    ...
    private boolean rayIntersection(int[] b, Vector3d rayOrigin, Vector3d rayDir) {
        Vector3d rInv = new Vector3d(1, 1, 1).div(rayDir);
     ...
     */
    private boolean isVisible(Vector3d start, Vector3d[] targets, int size) {
        ...
        for (int v = 0; v < size; v++) {
            ...
            if(allowRayChecks && rayIntersection(lastHitBlock, start, new Vector3d(relativeX, relativeY, relativeZ).normalize())) {
                continue;
            }
            ...
            if (dimensionX == 0f) {
                x_inc = 0;
                t_next_x = dimFracX; // don't increment horizontally because the Vector3d is perfectly vertical
            ...
            if (dimensionY == 0f) {
                y_inc = 0;
                t_next_y = dimFracY; // don't increment vertically because the Vector3d is perfectly horizontal
            ...
            if (dimensionZ == 0f) {
                z_inc = 0;
                t_next_z = dimFracZ; // don't increment vertically because the Vector3d is perfectly horizontal
    ...
    private boolean stepRay(Vector3d start, int currentX, int currentY,
