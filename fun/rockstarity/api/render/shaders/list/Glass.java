...
/**
 * @author ConeTin
 * @since 25 нояб. 2024 г.
 */

@FieldDefaults(level = AccessLevel.PRIVATE, makeFinal = true)
public class Glass extends Shader {
	
	static Glass shader = new Glass();
	@NonFinal static Framebuffer framebuffer = new Framebuffer(1, 1, false);
	...
	public static void draw(FixColor color, float noise, float reflected, float blur) {
	    ...
	    shader.start();
	    shader.setInt("inputSampler", 0);
        shader.setFloat("inputResolution", (float) mc.getFramebuffer().framebufferTextureWidth, (float) mc.getFramebuffer().framebufferTextureHeight);
        shader.setFloat("blurAmount", blur);
        shader.setFloat("reflect", reflected);
        shader.setFloat("noiseValue", noise*0.01f);
        shader.setFloat("vertexColor", ColorUtility.getRGBAFloat(color));
	}
