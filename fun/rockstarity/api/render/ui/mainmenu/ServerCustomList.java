...
public class ServerCustomList extends ExtendedList<ServerCustomList.Entry>
{
    ...
    private final MultiScreen owner;
    private final List<ServerCustomList.NormalEntry> serverListInternet = Lists.newArrayList();
    private final ServerCustomList.Entry lanScanEntry = new ServerCustomList.LanScanEntry();
    private final List<ServerCustomList.LanDetectedEntry> serverListLan = Lists.newArrayList();

    public ServerCustomList(MultiScreen ownerIn, Minecraft mcIn, int widthIn, int heightIn, int topIn, int bottomIn, int slotHeightIn)
    {
    ...
    public void setSelected(@Nullable ServerCustomList.Entry entry)
    {
        super.setSelected(entry);

        if (this.getSelected() instanceof ServerCustomList.NormalEntry)
        {
            NarratorChatListener.INSTANCE.say((new TranslationTextComponent("narrator.select", ((ServerCustomList.NormalEntry)this.getSelected()).server.serverName)).getString());
        }
    ...
    {
        ServerCustomList.Entry serverselectionlist$entry = this.getSelected();
    ...
    protected void moveSelection(AbstractList.Ordering p_241219_1_)
    {
        ...
        {
            return !(p_241612_0_ instanceof ServerCustomList.LanScanEntry);
        });
    }
    ...
    public void updateOnlineServers(ServerList p_148195_1_)
    {
        ...
        {
            this.serverListInternet.add(new ServerCustomList.NormalEntry(this.owner, p_148195_1_.getServerData(i)));
        }
    ...
    public void updateNetworkServers(List<LanServerInfo> p_148194_1_)
    {
        ...
        {
            this.serverListLan.add(new ServerCustomList.LanDetectedEntry(this.owner, lanserverinfo));
        }
    ...
    public abstract static class Entry extends ExtendedList.AbstractListEntry<ServerCustomList.Entry>
    {
    }

    public static class LanDetectedEntry extends ServerCustomList.Entry
    {
        ...
        protected LanDetectedEntry(MultiScreen p_i47141_1_, LanServerInfo p_i47141_2_)
        {
        ...
        public void render(MatrixStack p_230432_1_, int p_230432_2_, int p_230432_3_, int p_230432_4_, int p_230432_5_, int p_230432_6_, int p_230432_7_, int p_230432_8_, boolean p_230432_9_, float p_230432_10_)
        {
            ...
            if (this.mc.getGameSettings().hideServerAddress)
            {
    ...
    public static class LanScanEntry extends ServerCustomList.Entry
    {
        ...
        public void render(MatrixStack p_230432_1_, int p_230432_2_, int p_230432_3_, int p_230432_4_, int p_230432_5_, int p_230432_6_, int p_230432_7_, int p_230432_8_, boolean p_230432_9_, float p_230432_10_)
        {
            ...
            bold.get(17).draw(p_230432_1_, ServerCustomList.field_243365_r.getString(), (float)(this.mc.currentScreen.width / 2 - bold.get(17).getWidth(ServerCustomList.field_243365_r.getString()) / 2), (float)(i-10), rock.getThemes().getTextFirstColor().alpha(p_230432_10_));

           // if (!rock.isPanic()) {
            	float size = 5;
            	Loading.render(p_230432_1_, mc.currentScreen.width/2-size*2.5f, i+7, size);
            //}
            ...
            bold.get(17).draw(p_230432_1_, s, (float)(this.mc.currentScreen.width / 2 - bold.get(17).getWidth(s) / 2), (float)(i + 9), rock.getThemes().getTextSecondColor().alpha(p_230432_10_));
            */
        }
    }

    public class NormalEntry extends ServerCustomList.Entry
    {
        private final MultiScreen owner;
        ...
        protected NormalEntry(MultiScreen p_i50669_2_, ServerData p_i50669_3_)
        {
        ...
        public void render(MatrixStack p_230432_1_, int p_230432_2_, int p_230432_3_, int p_230432_4_, int p_230432_5_, int p_230432_6_, int p_230432_7_, int p_230432_8_, boolean p_230432_9_, float p_230432_10_)
        {
            if (!this.server.pinged)
            {
                ...
                ServerCustomList.field_214358_b.submit(() ->
                {
                    try {
                    ...
                    catch (UnknownHostException unknownhostexception)
                    {
                        ...
                        this.server.serverMOTD = ServerCustomList.field_243366_s;
                    }
                    catch (Exception exception)
                    {
                        this.server.pingToServer = -1L;
                        this.server.serverMOTD = ServerCustomList.field_243367_t;
                    }
                });
            }

            boolean flag = this.server.version != SharedConstants.getVersion().getProtocolVersion();
            bold.get(17).draw(p_230432_1_, this.server.serverName, (float)(p_230432_4_ + 32 + 3), (float)(p_230432_3_ + 1), rock.getThemes().getTextFirstColor().alpha(p_230432_10_));
            List<IReorderingProcessor> list = this.mc.fontRenderer.trimStringToWidth(this.server.serverMOTD, p_230432_5_ - 32 - 2);

            for (int i = 0; i < Math.min(list.size(), 2); ++i)
            {
                this.mc.fontRenderer.func_238422_b_(p_230432_1_, list.get(i), (float)(p_230432_4_ + 32 + 3), (float)(p_230432_3_ + 12 + 9 * i), new FixColor(8421504).alpha(Math.max(0.1, p_230432_10_)).getRGB());
            }
            ...
            String corrected = itextcomponent1.getString().contains(" ") ? itextcomponent1.getString().split(" ")[itextcomponent1.getString().split(" ").length-1] : itextcomponent1.getString();
            bold.get(15).draw(p_230432_1_, corrected, (float)(p_230432_4_ + p_230432_5_ - bold.get(15).getWidth(corrected) - 6), (float)(p_230432_3_), new FixColor(8421504).alpha(p_230432_10_));
            ...
            {
                l = 5;
                itextcomponent = ServerCustomList.field_244607_u;
            ...
            else if (this.server.pinged && this.server.pingToServer != -2L)
            {
                ...
                if (this.server.pingToServer < 0L)
                {
                    itextcomponent = ServerCustomList.field_243370_w;
                    list1 = Collections.emptyList();
                }
                else
                {
                    itextcomponent = new TranslationTextComponent( this.server.pingToServer >= 150 ? "multiplayer.status.ping_high" : "multiplayer.status.ping", this.server.pingToServer);
                    list1 = this.server.playerList;
                }
            }
            else
            {
                ...
                itextcomponent = ServerCustomList.field_243371_x;
                list1 = Collections.emptyList();
            }
            ...
            if (this.icon != null)
            {
                this.func_238859_a_(p_230432_1_, p_230432_4_, p_230432_3_, this.serverIcon, p_230432_10_);
            }
            else
            {
                this.func_238859_a_(p_230432_1_, p_230432_4_, p_230432_3_, ServerCustomList.field_214359_c, p_230432_10_);
            }
            ...
            if (this.mc.getGameSettings().touchscreen || p_230432_9_)
            {
                this.mc.getTextureManager().bindTexture(ServerCustomList.field_214360_d);
                Round.draw(p_230432_1_, new Rect(p_230432_4_, p_230432_3_, 32, 32), 3, FixColor.WHITE.alpha(0.5f));
        ...
        protected void func_238859_a_(MatrixStack p_238859_1_, int p_238859_2_, int p_238859_3_, ResourceLocation p_238859_4_, float p_230432_10_)
        {
            this.mc.getTextureManager().bindTexture(p_238859_4_);
            RenderSystem.enableBlend();
            Render.color(FixColor.WHITE.alpha(p_230432_10_));
            AbstractGui.blit(p_238859_1_, p_238859_2_, p_238859_3_, 0.0F, 0.0F, 32, 32, 32, 32);
            //Round.drawTextured(p_238859_1_, new Rect(p_238859_2_, p_238859_3_, 32, 32), 3, p_230432_10_);
        ...
        private boolean func_241614_a_(@Nullable String p_241614_1_)
        {
            ...
            else
            {
                ...
                {
                    ServerCustomList.LOGGER.error("Invalid icon for server {} ({})", this.server.serverName, this.server.serverIP, throwable);
                    return false;
                }
            }
        ...
        public boolean keyPressed(int keyCode, int scanCode, int modifiers)
        {
            ...
            {
                ServerCustomList serverselectionlist = this.owner.serverListSelector;
        ...
        private void func_228196_a_(int p_228196_1_, int p_228196_2_)
        {
            ...
            ServerCustomList.Entry serverselectionlist$entry = this.owner.serverListSelector.getEventListeners().get(p_228196_2_);
            this.owner.serverListSelector.setSelected(serverselectionlist$entry);
            ServerCustomList.this.ensureVisible(serverselectionlist$entry);
        }

        public boolean mouseClicked(double mouseX, double mouseY, int button)
        {
            double d0 = mouseX - (double)ServerCustomList.this.getRowLeft();
            double d1 = mouseY - (double)ServerCustomList.this.getRowTop(ServerCustomList.this.getEventListeners().indexOf(this));
