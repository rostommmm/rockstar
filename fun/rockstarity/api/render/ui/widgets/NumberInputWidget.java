...
public class NumberInputWidget extends Widget implements IRenderable, IGuiEventListener, IAccess
{
    private final FontSize fontRenderer;
    ...
    @Setter @Getter private String endText;
    @Setter @Getter private int min = - 999_999_999, max = 999_999_999;
    ...
    public NumberInputWidget(FontSize p_i232260_1_, int p_i232260_2_, int p_i232260_3_, int p_i232260_4_, int p_i232260_5_, ITextComponent p_i232260_6_)
    {
        this(p_i232260_1_, p_i232260_2_, p_i232260_3_, p_i232260_4_, p_i232260_5_, (NumberInputWidget)null, p_i232260_6_);
    }
    
    public NumberInputWidget(FontSize p_i232260_1_, int p_i232260_2_, int p_i232260_3_, int p_i232260_4_, int p_i232260_5_, ITextComponent p_i232260_6_, boolean bg)
    {
        this(p_i232260_1_, p_i232260_2_, p_i232260_3_, p_i232260_4_, p_i232260_5_, (NumberInputWidget)null, p_i232260_6_);
        this.bg = bg;
    }

    public NumberInputWidget(FontSize p_i232259_1_, int p_i232259_2_, int p_i232259_3_, int p_i232259_4_, int p_i232259_5_, @Nullable NumberInputWidget p_i232259_6_, ITextComponent p_i232259_7_)
    {
    ...
    private void onTextChanged(String newText)
    {
        ...
        if (getFloat() < min) this.text = min + "";
        if (getFloat() > max) this.text = max + "";
    }
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers)
    {
        ...
        else
        {
            ...
            else if (Screen.isPaste(keyCode))
            {
                if (this.isEnabled)
                {
                    this.writeText(mc.keyboardListener.getClipboardString());
                }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button)
    {
        ...
        else
        {
            ...
            if (this.isFocused() && flag && button == 0)
            {
                ...
                String s = mc.fontRenderer.func_238412_a_(this.text.substring(this.lineScrollOffset), this.getAdjustedWidth());
                this.setCursorPosition(mc.fontRenderer.func_238412_a_(s, i).length() + this.lineScrollOffset);
                return true;
            }
            else
            {
    ...
    {
    	this.renderButton(matrixStack, mouseX, mouseY, partialTicks, 1);
    }

    public void renderButton(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks, float anim)
    {
        if (this.getVisible())
        {
            if (this.getEnableBackgroundDrawing() && bg)
            {
                int i = this.isFocused() ? -1 : -6250336;
                //Round.drawRound(x, y, width, height, 5, ColorSystem.applyOpacity(RectTheme.getCurrent().getMainColor(), anim * (isFocused() ? 1 : Hover.isHovered(x, y, width, height, mouseX, mouseY) ? 0.8f : 0.6f)));
            }
            
            int i2 = this.isEnabled ? this.enabledColor : this.disabledColor;
            int j = this.cursorPosition - this.lineScrollOffset;
            int k = this.selectionEnd - this.lineScrollOffset;
            int lineScrollOffset = Math.min(this.lineScrollOffset, this.text.length());
            String s = mc.fontRenderer.func_238412_a_(this.text.substring(lineScrollOffset), this.getAdjustedWidth());
            ...
            if (!s.isEmpty())
            {
                ...
                fontRenderer.draw(matrixStack, s1, (float)l+1, (float)i1-2, rock.getThemes().getTextFirstColor().alpha(anim));
                j1 = (int) (l+1 + fontRenderer.getWidth(s1));
            }
            ...
            {
            	fontRenderer.draw(matrixStack, s.substring(j), (float)j1+3, (float)i1-2, rock.getThemes().getTextFirstColor().alpha(anim));
            }

            if (!flag2 && this.suggestion != null)
            {
            	fontRenderer.draw(matrixStack, this.suggestion, (float)(k1 - 1), (float)i1, rock.getThemes().getTextFirstColor().alpha(anim));
            }
            ...
            if (flag1)
            {
                if (flag2)
                {
                    AbstractGui.fill(matrixStack, k1 + 2, i1 - 1, k1 +3, i1 + 1 + 7, rock.getThemes().getTextFirstColor().alpha(anim).getRGB());
                }
                else
                {
                	fontRenderer.draw(matrixStack, "_", (float)k1+2, (float)i1-3, rock.getThemes().getTextFirstColor().alpha(anim));
                }
            }
            ...
            if (k != j)
            {
                ...
                //this.drawSelectionBox(k1, i1 - 1, l1 - 1, i1 + 1 + 7);
            }
            
            fontRenderer.draw(matrixStack, endText, l + fontRenderer.getWidth(text) + 1, i1 - 2, rock.getThemes().getTextFirstColor().alpha(anim));
        }
    }
    ...
    public void setSelectionPos(int position)
    {
        ...
        if (this.fontRenderer != null)
        {
            ...
            String s = mc.fontRenderer.func_238412_a_(this.text.substring(this.lineScrollOffset), j);
            int k = s.length() + this.lineScrollOffset;

            if (this.selectionEnd == this.lineScrollOffset)
            {
                this.lineScrollOffset -= mc.fontRenderer.func_238413_a_(this.text, j, true).length();
            }
    ...
    {
        return p_195611_1_ > this.text.length() ? this.x : this.x + mc.fontRenderer.getStringWidth(this.text.substring(0, p_195611_1_));
    }
